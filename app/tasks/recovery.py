...
def _insert_action_scheduled_outbox(
    session,
    case: RecoveryCase,
    action,
    deliver_at,
    ...
    outbox = OutboxEvent(
        ...
        event_type="recovery_case.action_scheduled",
        payload={
            "recovery_case_id": str(case.id),
            "recovery_action_id": str(action.id),
        },
        idempotency_key=f"recovery_case.action_scheduled:{case.id}:{action.id}",
...
def _handle_recovery_case_eligible(session, outbox: OutboxEvent) -> dict:
    ...
    if (
        selected_action == policy_decision.final_action
        and agent_decision.source != "llm"
    ):
        ...
        candidate_delay = (
            engine_decision.delay_hours
            if selected_action == RecoveryActionType.WAIT
            else None
        )

        final_policy_decision = evaluate_policy(
            limits=limits,
            policy_input=final_policy_input,
            candidate_delay_hours=candidate_delay,
        )

        if (
            final_policy_decision.approved
            and final_policy_decision.final_action == selected_action
        ):
            final_action = selected_action
    ...
    if final_status == RecoveryCaseStatus.ACTION_SCHEDULED:
        _insert_action_scheduled_outbox(
            session,
            case,
            action,
            next_action_at,
    ...
    _insert_decision_audit(
        ...
        deterministic_action=policy_decision.final_action,
        agent_action=agent_decision.selected_action,
        agent_source=agent_decision.source,
    ...
    return {
        ...
        "deterministic_action": policy_decision.final_action.value,
        "agent_action": agent_decision.selected_action.value,
        "agent_source": agent_decision.source,
        "agent_reason": agent_decision.reason,
        "final_action": final_action.value,
        "final_policy_approved": final_policy_approved,
...
def _process_eligible_event(outbox_event_id: str) -> dict:
    try:
...
def _process_outbox_event(outbox_event_id: str) -> dict:
    ...
    if event_type == "recovery_case.eligible":
        return _process_eligible_event(outbox_event_id)

    if event_type == "recovery_case.action_scheduled":
        from app.services.action_executor import execute_scheduled_action_from_outbox

        return execute_scheduled_action_from_outbox(payload)
